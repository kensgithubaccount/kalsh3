...
def main() -> None:
    ...
    if markets.get("market_count") == 0:
        ...
        print(
            "fresh KXHIGHCHI market discovery succeeded with complete pagination; contract settlement location identifier remains CLIMDW; "
            f"evidence_sha256={hashlib.sha256(args.public_evidence.read_bytes()).hexdigest()}"
        )
        return
    print("ABSTAIN_NO_VALID_FORECAST")
    print("fresh active KXHIGHCHI market exists; contract settlement location identifier remains CLIMDW; no reviewed current forecast bundle was supplied")
