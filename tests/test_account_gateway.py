...
@pytest.mark.parametrize("subaccount", [1, -1, "0", True, False, [], {}])
def test_read_key_rejects_nonprimary_or_malformed_restriction(
...
@pytest.mark.parametrize("restriction", ["omitted", None, 0])
def test_read_key_accepts_unrestricted_or_primary_restriction_and_targets_account_zero(
    private_key: bytes, restriction: str | int | None
) -> None:
    key: dict[str, Any] = {"api_key_id": "reader", "scopes": ["read"]}
    if restriction != "omitted":
        key["subaccount"] = restriction
    transport = Transport({"api_keys": [HttpResponse(200, {"api_keys": [key]})]})
    client(private_key, transport).refresh("reader")
    assert all("subaccount=0" in path for path in transport.paths if "/portfolio/" in path)
