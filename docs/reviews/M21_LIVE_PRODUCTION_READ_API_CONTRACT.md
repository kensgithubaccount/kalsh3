...
- `GET /trade-api/v2/api_keys` returns `api_keys` items with `api_key_id`, `name`, `scopes`, and
  optional `subaccount`. M22 corrects enrollment to distinguish credential restriction from runtime account
  targeting: an omitted/null restriction is unrestricted, while integer `0` explicitly restricts the key to
  the primary account. Exactly one `api_key_id` must match and scopes must equal exactly `["read"]`;
  duplicate, ambiguous, expanded, write-capable, nonzero, boolean, string, or malformed restrictions fail
  closed. Regardless of credential restriction, every portfolio request explicitly targets subaccount 0.
