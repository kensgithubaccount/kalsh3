...
`evidence/cpi_p9a_historical_price/`: the exact acquisition receipt, market-
inventory response, frozen manifest, and one raw JSON response per sibling.
...
Tracked size is 51,135,903 bytes uncompressed: 474 raw candle files
(48,423,949 bytes), the 891,842-byte inventory response, the 625,145-byte
acquisition receipt, and the 1,194,967-byte frozen manifest. The largest candle
response is 524,917 bytes.

Approved acquisition semantic digest:
...
The exact original runtime acquisition receipt is tracked at
`acquisition_manifest.json`: 625,145 bytes with raw SHA-256
`1ce8c675191aa15ae8f89c4f6a560ceaabdfd5c5bc4eb84289f45c092bf9de56`.
Its internal and independently recomputed approved semantic digest is
`d671ef2cda78a8e1a720126a73fed4e0228afc69bd72c86878bdcd5acbfc6699`.
Replay requires this fixed digest, the exact KXCPI-filtered inventory request,
the exhausted cursor, and receipt identities to agree with the frozen package.
...
Focused P9A plus historical-client and executable-quote tests: **41 passed**.

After the P9A.1 repair, the expanded offline replay/adversarial suite:
**41 passed**. It covers inventory mutation/deletion, immutable acquisition
receipt mutation/replacement, request reconstruction,
