...
| M27C Part 2B2 Physical-Temperature Proxy Probabilities | Model mechanics implemented and fixture-tested; empirical performance not yet accepted; research-only | Pure horizon-specific unweighted empirical residual models support only CLIMDW/KMDW/USW00014819 DAILY_MAX post-2020 YGUZ98 at exact 15h/39h/63h midpoints. Strict typed coverage loading, provenance-bound current GRIB evidence, nearest-rank intervals, exact CRPS, locked chronological splits, and nine exact Bonferroni-adjusted equal-tail binomial coverage gates fail closed. The frozen policy is family-wise alpha 1/20, 9 gates, per-test alpha 1/180, two-sided tail alpha 1/360, with equality rejected and adjustment method BONFERRONI; these fields are identity material. The 365-row per-horizon TRAIN minimum is a v1 operating safety floor, not statistical adequacy. Outputs are GHCNd physical-temperature proxies with unvalidated TWC settlement mapping; no Forecast, market comparison, edge, EV, risk, or execution integration. Full multi-year evaluation remains required; production influence exactly 0. See `reviews/M27C_WEATHER_CALIBRATED_PROBABILITIES.md`. |
