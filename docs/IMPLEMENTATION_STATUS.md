...
| M25 Live Read-Only Evidence Collection | M25A + M25B1 complete (offline verified) | M25A remains the Predictions-shaped offline YES/NO runtime. M25B1 adds a parallel ticker-only Perps metadata/parser, ordinary bid/ask sequenced book, restricted margin protocol, separate book and ticker/funding evidence, and three append-only Perps tables. Ticker replay identity uses epoch/SID/ticker/exchange timestamp/source fingerprint and excludes local clocks. Gaps, crossed books, and stale books require reconnect/new epoch/fresh snapshot; empty current books wait without claiming health. Perps book quantity granularity is exact 0.01 regardless of the retained fractional metadata flag. No M25A rows are reinterpreted. No networking, credentials, WebSocket dependency, deployment, or live collector. M25B2 remains future; production influence is exactly zero. |
