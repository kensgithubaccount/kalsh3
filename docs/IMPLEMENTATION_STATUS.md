...
| M25 Live Read-Only Evidence Collection | M25A complete (offline verified) | Disabled-by-default, single-consumer scripted runtime from transport-timestamped raw frames through strict parsing and canonical M24 evidence persistence. Official subscribed fixtures drive protocol/runtime state; stale snapshots cannot create false health; delta-before-snapshot and unknown-SID paths fail closed; multi-ticker SID recovery requires valid evidence for every affected ticker with one deduplicated request. Epoch isolation, reconnect, quarantine, immutable health, safety isolation, and hardened SQLite remain deterministic. No networking, credentials, deployment activation, or live collector. Production influence remains exactly zero. M25B live read-only metadata/collector work remains future. |
