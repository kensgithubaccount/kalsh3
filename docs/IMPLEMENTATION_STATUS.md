...
| M25 Live Read-Only Evidence Collection | M25A complete (offline verified) | Disabled-by-default, single-consumer scripted runtime from transport-timestamped raw frames through strict parsing and canonical M24 evidence persistence. Epoch isolation, reconnect, gap/resnapshot deduplication, quarantine, immutable health, safety isolation, and hardened SQLite are deterministic. Focused pytest: 59 passed, 1 unrelated signer test deselected; full suite: 456 passed with the same 46 tracked macOS signer portability failures. Ruff and targeted strict mypy passed. No networking, credentials, deployment activation, or live collector. Production influence remains exactly zero. M25B live read-only metadata/collector work remains future. |
