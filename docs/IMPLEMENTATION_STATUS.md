...
| CPI-E1-P7 Settlement Reconciliation | Implemented offline; empirical Kalshi cohort UNKNOWN | Fail-closed exact ContractSpecification/P6/raw-Kalshi binding, Decimal-only comparator evaluation, MATCHED/MISMATCH SettlementRecord construction, and adversarial tests. No exact immutable historical Kalshi market/rules/finalization artifacts are present in this clone; no empirical MATCHED record or G3 generalization claimed. Research-only with production influence 0. See `reviews/CPI_E1_P7_SETTLEMENT_RECONCILIATION.md`. |
