...
| M24 Perps Shadow Research | Part A complete (offline verified) | Immutable observations plus canonical snapshot/delta book evidence, explicit epoch/SID/sequence/exchange-index provenance, deterministic identity, and append-only SQLite storage. Focused pytest 31 passed; Ruff passed. Existing realtime coverage: 11 passed with one unrelated tracked macOS `/proc/self/fd` signer failure. Live collection OFF; production influence NONE. |
