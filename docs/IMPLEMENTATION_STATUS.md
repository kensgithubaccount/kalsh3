...
| M24 Perps Shadow Research | Complete (targeted offline verified) | Focused pytest 12 passed; Ruff lint and format checks passed; full-repo mypy locally blocked only by the pre-existing macOS `memfd_create` typing/platform issue in `services/production_execution/security_boundary.py`. Research-only immutable observations, separate directional leverage, nullable portfolio margin, fail-closed UTC edge decay; production influence NONE |
