...
| M25 Live Read-Only Evidence Collection | M25A + M25B1 + M25B2 + M25B3 implemented; live acceptance pending | M25B3 adds authenticated encrypted-at-rest, environment-proven, DEMO-only exact-read credential enrollment, verification, and provider composition. Its separate master-key and credential files share one user-state/OS-user boundary: filesystem permissions are the primary local boundary, and access to both files permits credential recovery; no same-user, root, combined-backup/snapshot, KMS, HSM, or keychain protection is claimed. No credential is enrolled or verified and no live smoke was run. Production composition remains BLOCKED. M25B2 stays manual/read-only with only orderbook_delta/ticker; no trading/write capability exists and production influence is exactly zero. |
