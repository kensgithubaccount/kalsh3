...
| M25 Live Read-Only Evidence Collection | M25A + M25B1 + M25B2 + M25B3 + M25B4 implemented; live acceptance pending | M25B4 makes the environment-neutral GET/HEAD-only `RequestSigner` portable across the project's normal macOS and Linux targets by transferring its in-memory key to OpenSSL through a narrowly inherited `/dev/fd` pipe. macOS provides `/dev/fd` natively; mainstream Linux commonly resolves it through `/proc/self/fd`, preserving prior Linux behavior and retaining the normal procfs assumption rather than claiming support for minimal procfs-free Linux environments. No plaintext key file, write capability, environment binding, or production composition was added. M25B3 remains DEMO-only with encrypted-at-rest, environment-proven credential enrollment, verification, and provider composition. No credential is enrolled or verified and no live smoke was run. Production composition remains BLOCKED; production influence is exactly zero. |
