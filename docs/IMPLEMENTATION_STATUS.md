...
| CPI-E1-P7 Settlement Reconciliation | Implemented; three exact public historical KXCPI markets empirically MATCHED | Fail-closed exact ContractSpecification/P6/raw-Kalshi binding, Decimal-only comparator evaluation, explicit historical `status=finalized` plus settlement timestamp, MATCHED/MISMATCH SettlementRecord construction, and adversarial tests. Public unauthenticated GET evidence is recorded by endpoint/hash; historical API exposes no separate rules-version field, so content-addressed market-rules identity remains an independent-review limitation. Research-only with production influence 0. See `reviews/CPI_E1_P7_SETTLEMENT_RECONCILIATION.md`. |
