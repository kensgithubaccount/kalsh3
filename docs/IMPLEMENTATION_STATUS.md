...
| M9-E1 Learning Governance Semantic Repair | Implemented; focused regressions and full suite verified; research-only | `paired_event_interval()` treated leave-one-event-out extrema as an inferential interval and could emit `STRONGER_EVIDENCE`; duplicate `event_id`s inflated the denominator; all-negative contributions promoted; `GovernanceProposal` trusted a supplied `unique_settled_events` count and required no positive incremental effect. All four were reproduced on `ca72cb2b38b2ea11f4f3650a9ad31d9b5c348d2f` before repair. Extrema are now named `sensitivity_low`/`sensitivity_high` and carry `method=LEAVE_ONE_EVENT_OUT_SENSITIVITY` with `inferential_method=None`; evidence is `INCONCLUSIVE` by construction and `STRONGER_EVIDENCE` is unconstructible without a prespecified method handling dependence and repeated looks. Duplicate ids fail closed, the 50-unique-settled-event floor may be raised but never lowered, promotion binds `unique_settled_events` to an exact duplicate-free `event_manifest` and to the interval's own `event_count`, and a strictly positive after-cost/market-relative `incremental_effect` plus a positive observed direction are required independently of evidence state. `compare_challenger()` is presently always `False` by construction. Migrations and stored records are unmodified; artifacts remain frozen. No production execution, credential, risk-limit, trading-activation, or frozen GDP/weather semantics change; production influence exactly 0. See `reviews/M9E1_LEARNING_GOVERNANCE_SEMANTIC_REPAIR.md`. |
...
- Statistical uncertainty above is superseded by M9-E1: leave-one-event-out extrema are a
  sensitivity diagnostic, not an interval, and promotion evidence is INCONCLUSIVE by construction
  until a prespecified inferential method for event dependence and repeated looks exists. The
  champion/challenger and human-gated promotion paths are therefore fail-closed and currently
  cannot promote. See `reviews/M9E1_LEARNING_GOVERNANCE_SEMANTIC_REPAIR.md`.
