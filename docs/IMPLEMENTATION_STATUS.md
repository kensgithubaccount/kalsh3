...
| GDP Authority Integration Preflight | PR #157 closure repair follows rejected alias repair; current-head CI and independent acceptance required | Registration and replay capture the original EvaluationPlan in one import-time closure; regression cases cover public/private/both names, unchanged durable state on rejection, and clean reopen. Fee completeness remains unavailable; next step is a bounded GDP evidence feasibility disposition. See `reviews/GDP_EVALUATION_PLAN_REPAIR.md`. |
