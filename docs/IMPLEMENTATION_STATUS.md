...
| M25 Live Read-Only Evidence Collection | M25A + M25B1 + M25B2 implemented; live acceptance pending | M25B2 adds a manual-only, explicitly enabled Perps read boundary: fixed demo/production origins, public market metadata GET, signed enabled GET, authenticated margin WebSocket, raw receipt timestamps, connection epochs, bounded reconnect, and only orderbook_delta/ticker. The current encrypted read vault lacks environment provenance, so runtime credential composition remains BLOCKED behind an injectable exact-read provider. No live smoke was run. No trading/write capability exists; production influence is exactly zero. |
